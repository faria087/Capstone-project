...
export const AffectedTypeList = () => {
    ...
    const deleteHandler = async (id) => {
        await axios.delete(`http://localhost:8000/api/affectedtypes/${id}`);
        const response = await axios.get("http://localhost:8000/api/affectedtypes");
        if (response?.data?.success) {
            setAffectedTypes(response.data.data);
        }
        ...
        <div className="admin">
            ...
            <div className="admin-content">
                ...
                <div className="admin-content-main">
                    <a href="/be/affectedtypes-create" className="button">Add Affected Type</a>
                    <div className="aid-control">
                        <h1>Affected Types </h1>
                        <table>
                            <thead>
                                <tr>
                                    <th>SL NO</th>
                            ...
                            <tbody>
                                {affectedtypes?.slice(0, visibleCount).map((affectedtype, index) => (

                                    <tr key={affectedtype._id} style={{ height: "50px" }}>
                                        <td>{index + 1}</td>
                                        <td>{affectedtype.title}</td>
                                        <td >
                                            <Link to={`/be/affectedtypes-edit/${affectedtype.id}`} style={{ background: "white", color: "black", width: "50px", height: "50px", padding: "15px", borderRadius: "50%", marginRight: "8px" }}><FontAwesomeIcon icon={faEdit} /></Link>
                                            <button style={{ background: "white", color: "black", width: "50px", height: "50px", padding: "15px", borderRadius: "50%" }} type="button" onClick={() => deleteHandler(affectedtype.id)} ><FontAwesomeIcon icon={faTrashAlt} /></button>
                                        </td>
                                    </tr>
                                ))}
                        ...
                        <div style={{ marginTop: "20px", textAlign: "center" }}>
                            {visibleCount < affectedtypes.length && (
                                <button
                                    className="button"
