    ...
    React.useEffect(() => {
        ...
        const fetchGallaries = async () => {
            try {
                ...
                if (response.data.success) {
                    setGallaries(response.data.data);
                }
            } catch (error) {
        ...
        <div className="admin">
            ...
            <div className="admin-content">
                ...
                <div className="admin-content-main">
                    <a href="/be/gallaries-create" className="button">Add Gallary</a>
                    <div className="aid-control">
                        ...
                        <table>
                            ...
                            <tbody>
                                {gallaries?.slice(0, visibleCount).map((gallary, index) => (

                                    <tr key={gallary._id} style={{ height: "50px" }}>
                                        <td>{index + 1}</td>
                                        <td>
                                            <img src={`http://localhost:8000/${gallary.image}`} alt="Landing Card" style={{ width: "50px", height: "50px" }} />
                                        </td>
                                        <td>{moment(gallary?.created_at).fromNow()}</td>
                                        <td >
                                            <Link to={`/be/gallaries-edit/${gallary.id}`} style={{ background: "white", color: "black", width: "50px", height: "50px", padding: "15px", borderRadius: "50%", marginRight: "8px" }}><FontAwesomeIcon icon={faEdit} /></Link>
                                            <button style={{ background: "white", color: "black", width: "50px", height: "50px", padding: "15px", borderRadius: "50%" }} type="button" onClick={() => deleteHandler(gallary.id)} ><FontAwesomeIcon icon={faTrashAlt} /></button>
                                        </td>
                                    </tr>
                                ))}
