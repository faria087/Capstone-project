           ...
           React.useEffect(() => {
            ...
            const toggleSidebar = () => {
                ...
                const deleteHandler = async(id) =>{
                    try {
                        const response = await axios.delete(`http://localhost:8000/api/activities/${id}`);
                        if (response.data.success) {
             ...
             <div className="admin">
                ...
                <div className="admin-content">
                    ...
                    <div className="admin-content-main">
                        <a href="/be/activities-create"  className="button">Add Activity</a>
                                ...
                                <tbody>
                                    {activities.map((activity, index) => (
                                        <tr key={activity._id}>
                                            ...
                                            <td style={{width:"30%"}}>{activity.description}</td>
                                            <td>
                                                <img src={`http://localhost:8000/${activity.image}`} alt="Landing Card"  style={{width:"100px",height:"100px"}}/>
                                            </td>
                                            <td>{moment(activity?.created_at).fromNow()}</td>
                                            <td>
                                            <Link to={`/be/activities-edit/${activity.id}`} style={{background:"white",color:"black",width:"50px",height:"50px",padding:"15px",borderRadius:"50%",marginRight:"8px"}}><FontAwesomeIcon icon={faEdit} /></Link>
                                                <button type="button" onClick={()=>deleteHandler(activity.id)}  style={{width:"50px",height:"50px",background:"#ffff",borderRadius:"50%",color:"black",marginRight:"10px"}}><FontAwesomeIcon icon={faTrashAlt} /></button>
                                            </td>
                                        </tr>
                                    ))}
                                </tbody>
