...
export const TCenterEditPage = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            const response = await axios.get(`http://localhost:8000/api/tcenters/${id}`);
            if (response?.data?.success) {
                setTitle(response?.data?.data?.title);
                setLocation(response?.data?.data?.location);
                setDescription(response?.data?.data?.description);
                setOpeningHours(response?.data?.data?.opening_hours);
                setClosingHours(response?.data?.data?.closing_hours);
                setCenterType(response?.data?.data?.center_type);
    ...
    const openingHoursChangeHandeler = (e) => {
        setOpeningHours(e.target.value);
    }

    const closingHoursChangeHandeler = (e) => {
        setClosingHours(e.target.value);
    }

    const centerTypeChangeHandeler = (e) => {
    ...
    const submitHandeler = async (e) => {
        ...
        await axios.post(`http://localhost:8000/api/tcenters/${id}`, formData)
            .then((response) => {
                if (response?.data?.success) {
                    navigate('/be/tcenters')
                    toast.success('TCenter updated successfully!', {
                            ...
                            <form action="" onSubmit={submitHandeler}>
                                ...
                                <div className="form-group">
                                 ...
                                 <div className="form-group">
                                    ...
                                    <textarea id="description" name="description" value={description} onChange={descriptionChangeHandeler} />
                                 </div>
                                <div className="form-group">
                                    <label htmlFor="opening_hours">Opening Hours</label>
                                    <input type="time" id="opening_hours" name="opening_hours" value={opening_hours} onChange={openingHoursChangeHandeler} />
                                </div>
                                <div className="form-group">
                                    <label htmlFor="closing_hours">Closing Hours</label>
                                    <input type="time" id="closing_hours" name="closing_hours" value={closing_hours} onChange={closingHoursChangeHandeler} />
                                </div>
                                <div className="form-group">
                                    <label htmlFor="center_type">Center Type</label>
                                    <input type="text" id="center_type" name="center_type" value={center_type} onChange={centerTypeChangeHandeler} />
                                </div>
                                <div className="form-group">
